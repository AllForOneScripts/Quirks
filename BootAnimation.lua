...
-- Envolvemos todo en task.spawn para evitar bloquear la carga del Hub
task.spawn(function()
    ...
    -- Asegurado con pcall para que no rompa el script si el ejecutor es limitado
    ...
    local resourcePath = GetCustomResource("Cosmic.mp3", AudioAssetURL)
    
    if resourcePath ~= "" then
        snd.SoundId = resourcePath
        snd.Volume = 2
    end
