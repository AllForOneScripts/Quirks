...
local function SpawnAFODimension(centerCF)
    ...
    local function createSinisterSmoke(polePart, emitDirection)
        ...
        smokeEmitter.Color = ColorSequence.new({ColorSequenceKeypoint.new(0, AFO_DEEP_PURPLE), ColorSequenceKeypoint.new(1, AFO_BLACK)})
        smokeEmitter.Size = NumberSequence.new({NumberSequenceKeypoint.new(0, 10), NumberSequenceKeypoint.new(1, 40)})
        smokeEmitter.Transparency = NumberSequence.new({NumberSequenceKeypoint.new(0, 1), NumberSequenceKeypoint.new(0.3, 0.85), NumberSequenceKeypoint.new(1, 1)})
    ...
    southParticles.Color = ColorSequence.new({ColorSequenceKeypoint.new(0, AFO_BLACK), ColorSequenceKeypoint.new(0.5, AFO_FUCHSIA), ColorSequenceKeypoint.new(1, AFO_DEEP_PURPLE)})
    ...
    southParticles.Transparency = NumberSequence.new({NumberSequenceKeypoint.new(0, 1), NumberSequenceKeypoint.new(0.2, 0.8), NumberSequenceKeypoint.new(0.8, 0.8), NumberSequenceKeypoint.new(1, 1)})
    ...
    conn = RunService.RenderStepped:Connect(function()
        if not dimensionFolder.Parent then if conn then conn:Disconnect() end return end
        ...
        hazeEmitter.Transparency = NumberSequence.new({NumberSequenceKeypoint.new(0, 1), NumberSequenceKeypoint.new(0.5, softTrans), NumberSequenceKeypoint.new(1, 1)})
        ...
        for _, tex in ipairs(allTextures) do
            if tex.Name == "NeonMain" or tex.Name == "NeonGlow" then
                if tex.Parent.Name == "Top" or tex.Parent.Name == "Bottom" then tex.OffsetStudsV = offsetNeon else tex.OffsetStudsU = offsetNeon end
            ...
            elseif tex.Name == "GlitchMask" then
                ...
                if tex.Parent.Name == "Top" or tex.Parent.Name == "Bottom" then tex.OffsetStudsV = glitchOffset tex.OffsetStudsU = glitchOffset else tex.OffsetStudsU = glitchOffset tex.OffsetStudsV = glitchOffset end
            end
        end
    end)
end
