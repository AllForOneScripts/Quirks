...
-- Full MUI visual signature for the clone. Particles are attached directly to
-- every visible body part; no rings, orbiting parts, or root-only aura exists.
-- All instances are parented under
...
local function MUICreateDivineCloneVFX()
    ...
    for _, MUIPart in ipairs(MUICloneModel:GetDescendants()) do
        if MUIPart:IsA("BasePart")
            ...
            local MUIC1Multiplier = 1
            if MUIName:find("torso") then MUIScale *= 1.6 end
            if MUIName:find("head") then MUIScale *= 1.2 end
            if MUIArm then
                MUIScale *= 1.45
                MUIC1Multiplier = 0.5
            end
            ...
            MUIEmitter("C1_LiquidEdge", ColorSequence.new(Color3.fromRGB(220, 245, 255)), 20,
                ...
                NumberSequence.new(
                    0.9 * MUIScale * MUIC1Multiplier,
                    1.2 * MUIScale * MUIC1Multiplier,
                    0.5 * MUIScale * MUIC1Multiplier
                ))
...
local function MUICreateLine(MUIThreat)
    ...
    MUIWarningGradient.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0.00, Color3.fromRGB(255, 210, 25)),
        ColorSequenceKeypoint.new(0.245, Color3.fromRGB(255, 210, 25)),
        ColorSequenceKeypoint.new(0.25, Color3.fromRGB(18, 18, 18)),
        ColorSequenceKeypoint.new(0.495, Color3.fromRGB(18, 18, 18)),
        ColorSequenceKeypoint.new(0.50, Color3.fromRGB(255, 210, 25)),
        ColorSequenceKeypoint.new(0.745, Color3.fromRGB(255, 210, 25)),
        ColorSequenceKeypoint.new(0.75, Color3.fromRGB(18, 18, 18)),
        ColorSequenceKeypoint.new(1.00, Color3.fromRGB(18, 18, 18)),
    })
    -- The stripes cover the entire line thickness, rather than being a thin
    -- yellow inset inside a mostly black bar.
    MUIWarningGradient.Parent = MUIOuter
