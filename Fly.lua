...
local function startIdleWatcher()
    ...
    flyanim.idleTimer = task.spawn(function()
        while flyanim.enabled do
            ...
            -- Mega Up activo: no resetear a normal por ausencia de teclas
            if flyanim.megaTurboUpActive then continue end
...
-- ============================================================
-- DETECCIÓN DE SUELO SEGURO (para caída épica inmediata)
-- ============================================================
-- Lanza raycasts en abanico para hallar la posición más cercana
-- del suelo bajo el personaje. Devuelve Vector3 o nil si no encuentra.
local function findSafeGroundBelow(root, charCurrent)
    ...
    for _, off in ipairs(offsets) do
        ...
        if result and result.Normal.Y > 0.5 then
            ...
            if bestY == nil or hitY > bestY then bestY = hitY end
        end
    end
    if bestY == nil then return nil end
    -- Posicionar al personaje 3 studs sobre el suelo
    return Vector3.new(root.Position.X, bestY + 3, root.Position.Z)
end
...
local function _flyOff()
    ...
    -- ── CAÍDA ÉPICA CON TP INMEDIATO (altura ≥ 45 studs) ────────────────────
    -- Si el jugador está a ≥45 studs del suelo, teletransportarlo al suelo
    -- INMEDIATAMENTE y ejecutar la animación épica sin esperar que caiga
    -- libremente (que puede tardar varios segundos y se ve antinatural).
    -- LA CONDICIÓN DE TP ES LA PRUEBA LÓGICA DE QUE ES UNA CAÍDA ÉPICA.
    if capturedHeight >= 45 and rootCurrent then
        local safeGroundPos = findSafeGroundBelow(rootCurrent, charCurrent)
        if safeGroundPos then
            -- Limpiar motores residuales antes del TP
            pcall(function()
            ...
            pcall(function()
                local _, ry, _ = rootCurrent.CFrame:ToOrientation()
                if ry ~= ry then ry = 0 end  -- nan guard
                rootCurrent.CFrame = CFrame.new(safeGroundPos) * CFrame.Angles(0, ry, 0)
                rootCurrent.AssemblyLinearVelocity  = Vector3.new(0, 0, 0)
                rootCurrent.AssemblyAngularVelocity = Vector3.new(0, 0, 0)
            end)
            -- Segunda pasada de velocidad cero en defer
            local _capturedRoot = rootCurrent
            task.defer(function()
                if flyanim.enabled then return end
                if not _capturedRoot or not _capturedRoot.Parent then return end
                pcall(function()
                    _capturedRoot.AssemblyLinearVelocity  = Vector3.new(0, 0, 0)
                    _capturedRoot.AssemblyAngularVelocity = Vector3.new(0, 0, 0)
                end)
            end)
            -- Forzar estado Landed para asentar el humanoid
            pcall(function() hum:ChangeState(Enum.HumanoidStateType.Landed) end)
            -- Preparar animScript para doLanding
            do
                local animScriptEpic = charCurrent:FindFirstChild("Animate") or flyanim.animScript
                if animScriptEpic then
                    animScriptEpic.Disabled = false
                    flyanim.animScript = animScriptEpic
                end
            end
            -- Limpiar estado waitingLand: ya no se necesita esperar
            flyanim.waitingLand = false
            if flyanim.landConn then flyanim.landConn:Disconnect(); flyanim.landConn = nil end
            stopLandingWatcher()
            ...
            -- Ejecutar animación épica con pequeño delay para que el humanoid se asiente
            task.delay(0.05, function()
