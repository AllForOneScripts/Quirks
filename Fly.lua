...
local function stopAllExcept(keepId, fade, alsoKeepLevitacion)
    ...
    for id, t in pairs(flyanim.tracks) do
        if not t or not t.Parent then flyanim.tracks[id] = nil; continue end
...
local function normPlaySafe(track, fade)
    ...
    if track and track.Parent ~= nil and not track.IsPlaying then
...
local function normStopSafe(track, fade)
    fade = fade or ANIM_NORMAL.TRANSICION_FLUIDA
    if track and track.Parent ~= nil and track.IsPlaying then
        pcall(function() track:Stop(fade) end)
    end
end

-- ANIMACIONES DE MOVIMIENTO RESTAURADAS AL SISTEMA FLUIDO FLY290
...
-- LÓGICA DE MOVIMIENTO PURO SIN WATCHDOGS (DE FLY290)
...
-- ============================================================
-- FLY ON / OFF (ARREGLO SALVAJE APLICADO)
-- ============================================================
...
local function _flyOn()
    ...
    startAnomalyProtection(); startAnimBlockLoop()
