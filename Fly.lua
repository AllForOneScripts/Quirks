-- ═══════════════════════════════════════════════════════════════════════════
-- MÓDULO: Fly (versión monolítica final)
-- Contiene toda la lógica del vuelo sin dependencias externas.
-- ═══════════════════════════════════════════════════════════════════════════
print("version 1.66 - Fly monolítico corregido")
...
-- Referencias que se asignarán en M.Start
local lplr   = nil
local camera = nil

-- ============================================================
-- SISTEMA DE IDIOMA (lee AllForOne/lang.txt)
-- ============================================================
local FlyLang = {
    ES = {
        fly_title        = "  VUELO",
...
-- ============================================================
-- FUNCIONES AUXILIARES DE SONIDO Y EFECTOS
-- ============================================================
...
-- ============================================================
-- SISTEMA DE ANIMACIONES (flyanim.*)
-- ============================================================
...
local function actualizarPosicionNormal(offsetY, velocidad)
    if not flyanim.rootJoint or not flyanim.originalC0 then return end
    flyanim.heightTweenToken = (flyanim.heightTweenToken or 0) + 1
    local myToken = flyanim.heightTweenToken
    local targetCF = (offsetY == 0) and flyanim.originalC0 or flyanim.originalC0 + Vector3.new(0, offsetY, 0)
    ...
    task.spawn(function()
        while true do
            task.wait()
            if flyanim.heightTweenToken ~= myToken then return end
            if not flyanim.enabled then return end
...
-- ============================================================
-- ANIMACIÓN NORMAL (movimiento, idle, laterales)
-- ============================================================
...
local function evaluarMovimientoNormal()
    ...
    local function matarAtras() flyanim.currentAtrasId = (flyanim.currentAtrasId or 0) + 1; flyanim.atrasKilled = true end
...
function iniciarCicloNormal(thisPlay)
    local nt = flyanim.normalTracks
    ...
    actualizarPosicionNormal(ANIM_NORMAL.COMPENSACION_ALTURA, ANIM_NORMAL.TRANSICION_POS)
    task.spawn(function()
        local to = tick() + 3
...
function iniciarPoseTurbo()
    ...
    task.spawn(function()
        ...
        local function abortarYVolver()
            ...
            setNoclip(false); stopParticleEmitter()
            task.wait(0.05)
        ...
        if nt.turbo_compact then
            ...
            pcall(function() nt.turbo_compact:Play(0.05); nt.turbo_compact:AdjustSpeed(3.0) end)
        end
        local startT = tick()
        ...
        if nt.turbo_burst then
            ...
            pcall(function() nt.turbo_burst:Play(0.05); nt.turbo_burst:AdjustSpeed(2.5) end)
        end
        task.wait(0.2)
        ...
        if not flyanim.enabled then
            flyanim.turboTransitioning = false; flyanim.turboPreImpulsoActivo = false
            pcall(function() preImpulsoBlockConn:Disconnect() end)
            flyanim.c0ControlToken = (flyanim.c0ControlToken or 0) + 1
            detenerWatchdogAltura(); restaurarC0Inmediato()
            return
        ...
        flyanim.turboRenderConn = RunService.RenderStepped:Connect(function()
            if not flyanim.enabled or flyanim.mode ~= "fast" then
                if flyanim.turboRenderConn then flyanim.turboRenderConn:Disconnect(); flyanim.turboRenderConn = nil end
                if myC0TkLoop == flyanim.c0ControlToken then detenerWatchdogAltura(); restaurarC0Inmediato() end
                return
            end
            if myC0TkLoop ~= flyanim.c0ControlToken then return end
            if nt.turbo_pose and nt.turbo_pose.IsPlaying then pcall(function() nt.turbo_pose:AdjustSpeed(0); nt.turbo_pose.TimePosition = ANIM_TURBO.FRAME_CONGELADO end)
            elseif nt.turbo_pose and not nt.turbo_pose.IsPlaying then nt.turbo_pose.Looped = true; pcall(function() nt.turbo_pose:Play(0.1); nt.turbo_pose:AdjustSpeed(0) end) end
            if nt.turbo_volado and nt.turbo_volado.IsPlaying then pcall(function() nt.turbo_volado:AdjustSpeed(0); nt.turbo_volado.TimePosition = ANIM_TURBO.FRAME_VOLADO_CONGELADO end)
            elseif nt.turbo_volado and not nt.turbo_volado.IsPlaying then nt.turbo_volado.Looped = true; pcall(function() nt.turbo_volado:Play(0.1); nt.turbo_volado:AdjustSpeed(0) end) end
...
function iniciarPoseMega()
    ...
    flyanim.megaRenderConn = RunService.RenderStepped:Connect(function()
        if not flyanim.enabled or flyanim.mode ~= "turbo" then
            if flyanim.megaRenderConn then flyanim.megaRenderConn:Disconnect(); flyanim.megaRenderConn = nil end
            if myC0Token == flyanim.c0ControlToken then detenerWatchdogAltura(); restaurarC0Inmediato() end
            return
        end
        if myC0Token ~= flyanim.c0ControlToken then return end
        if nt.mega_volado and nt.mega_volado.IsPlaying then pcall(function() nt.mega_volado:AdjustSpeed(0); nt.mega_volado.TimePosition = ANIM_MEGA.FRAME_VOLADO_CONGELADO end)
        elseif nt.mega_volado and not nt.mega_volado.IsPlaying then nt.mega_volado.Looped = true; pcall(function() nt.mega_volado:Play(0.1); nt.mega_volado:AdjustSpeed(0) end) end
...
local function cambiarAEspacioSecundario()
    ...
    detenerNormalTracks(0.1)
    pcall(function() nt.espacio_sec:Play(0.1) end); pcall(function() nt.espacio_sec:AdjustSpeed(0) end)
...
function manejarEspacioPresionadoNormal()
    ...
    flyanim.isBrazosActive = false; flyanim.idleTimerAnim = 0
    flyanim.currentAtrasId = (flyanim.currentAtrasId or 0) + 1; flyanim.atrasKilled = true
    flyanim.currentLateralId = (flyanim.currentLateralId or 0) + 1; flyanim.lateralKilled = true
...
function manejarEspacioSoltadoNormal()
    ...
    if flyanim.noclipSpaceActive then
        ...
        if flyanim.mode ~= "turbo" and not flyanim.noclipCtrlActive then setNoclip(false) end
    end
...
local function finalizarCombo()
    ...
    if flyanim.mode == "normal" then
        ...
        local nt = flyanim.normalTracks
        if nt and nt.levitacion and not nt.levitacion.IsPlaying then pcall(function() nt.levitacion:Play(0.2); nt.levitacion:AdjustSpeed(ANIM_NORMAL.VELOCIDAD_LEVITACION) end) end
...
function handleComboClick()
    ...
    local now = tick()
    if now - (flyanim.comboLastClick or 0) < COMBO_MIN_GAP then return end
...
-- ============================================================
-- ACTUALIZACIÓN DE ANIMACIÓN SEGÚN MOVIMIENTO (fast/turbo)
-- ============================================================
...
local function doLanding(char)
    ...
    if altura < 10 then
        ...
        if root then
            for _, v in ipairs(root:GetChildren()) do
                if v:IsA("BodyGyro") or v:IsA("BodyVelocity") or v:IsA("BodyForce") then v:Destroy() end
            end
        end
        if humanoid then
    ...
    if root then
        ...
        pcall(function() root.CFrame = CFrame.new(root.Position) * CFrame.Angles(0, ry, 0); root.AssemblyAngularVelocity = Vector3.new(0,0,0) end)
    end
    ...
    if altura >= 25 then
        ...
        shakeCamera(2.0 + (math.clamp(altura,25,200)-25)/175*1.5, 0.30)
        ...
        pcall(function() landBase:Destroy(); landOverlay:Destroy() end); _landBaseRef = nil; _landOverlayRef = nil
    else
        shakeCamera(0.8 + (math.clamp(altura,10,25)-10)/15*1.2, 0.18)
        landAnimConn:Disconnect(); task.wait(0.3)
    end
...
-- ============================================================
-- RESTAURAR ANIMACIONES DEL JUEGO
-- ============================================================
function restoreGameAnimations(char)
    if not char then return end
    if flyanim.animConn then flyanim.animConn:Disconnect(); flyanim.animConn = nil end
    if flyanim.animBlockRenderConn then flyanim.animBlockRenderConn:Disconnect(); flyanim.animBlockRenderConn = nil end
    local animScript = char:FindFirstChild("Animate") or flyanim.animScript
...
-- ============================================================
-- PROTECCIONES (anti-impulso, anomalías, teleport guard, etc.)
-- ============================================================
local function startAntiImpulse()
    ...
    flyanim.antiImpulseConn = RunService.Stepped:Connect(function()
        ...
        local char = lplr.Character; local root = char and char:FindFirstChild("HumanoidRootPart")
        if not root then return end
        ...
        if (actualVel - expectedVel).Magnitude > 3 then root.AssemblyLinearVelocity = expectedVel end
        local bg = flyanim.bg
        if bg then
...
local function silentMotorReset()
    ...
    lastMotorReset = now
    local char = lplr.Character; local root = char and char:FindFirstChild("HumanoidRootPart"); local hum = char and char:FindFirstChildOfClass("Humanoid")
    ...
    flyanim.teleportGuardConn = RunService.Stepped:Connect(function(_, dt)
        ...
        if pos.Y <= -500 then
            ...
            if safePos then
                ...
                local cam = workspace.CurrentCamera; if cam then local camCF = cam.CFrame; task.defer(function() if cam and cam.Parent then cam.CFrame = camCF end end) end
                flyanim.dashTimer = 0; flyanim.dashVel = Vector3.new(0,0,0)
...
local function startBrakeSystem()
    ...
    brakeConn = RunService.RenderStepped:Connect(function()
        ...
        local char = lplr.Character; local myRoot = char and char:FindFirstChild("HumanoidRootPart")
        ...
        if dist <= flyanim.BRAKE_HARD_DISTANCE then flyanim.dashVel = Vector3.new(0,0,0); flyanim.dashTimer = 0; flyanim.brakingActive = false; showHitboxMarkers(targetChar)
...
local function stopBrakeSystem() if brakeConn then brakeConn:Disconnect(); brakeConn = nil end; flyanim.brakingActive = false end

local dashConnection = nil; local HIT_COOLDOWN = 0.5
local function startDashCollisionDetection()
    ...
    dashConnection = RunService.RenderStepped:Connect(function()
        ...
        local currentChar = lplr.Character; local currentRoot = currentChar and currentChar:FindFirstChild("HumanoidRootPart")
...
-- ============================================================
-- EFECTOS VISUALES (PARTÍCULAS, SHOCKWAVE, FLASH, ATERRIZAJE)
-- ============================================================
...
local function sonicBoomEffect(intensity)
    ...
    for i, r in ipairs(rings) do
        task.spawn(function()
            ...
            local outer = Instance.new("ImageLabel", sg); outer.Image=RING_ID; outer.Size=UDim2.new(1,0,1,0); outer.BackgroundTransparency=1; outer.ImageColor3=WHITE; outer.ImageTransparency=0.06; outer.ZIndex=3
            local innerScale = 0.72 - (i-1)*0.08
            local inner = Instance.new("ImageLabel", sg); inner.Image=RING_ID; inner.Size=UDim2.new(innerScale,0,innerScale,0); inner.Position=UDim2.new((1-innerScale)/2,0,(1-innerScale)/2,0); inner.BackgroundTransparency=1; inner.ImageColor3=WHITE; inner.ImageTransparency=0.20; inner.ZIndex=2
    ...
    task.spawn(function()
        ...
        local img = Instance.new("ImageLabel", bb); img.Image=SHOCKWAVE_ID; img.Size=UDim2.new(1,0,1,0); img.BackgroundTransparency=1; img.ImageColor3=WHITE; img.ImageTransparency=0; img.ScaleType=Enum.ScaleType.Fit
...
local startParticleEmitter, stopParticleEmitter
startParticleEmitter = function()
...
stopParticleEmitter = function() if flyanim.particleConn then task.cancel(flyanim.particleConn); flyanim.particleConn = nil end; flyanim.particleRunning = false end
local function killActiveParticles() local list = flyanim.particleList; for i = #list, 1, -1 do local ref = list[i]; if ref and ref.part then pcall(function() ref.part:Destroy() end); ref.part = nil end; table.remove(list, i) end end

function spawnLandingEffects(position, velocity)
    ...
    task.spawn(function()
        ...
        local char = lplr.Character; local hrpNow = char and char:FindFirstChild("HumanoidRootPart"); local samplePos = hrpNow and hrpNow.Position or position
                ...
                for i = 1, thisBatch do
                    task.spawn(function()
                        ...
                        task.delay(0.04, function()
                            ...
                            TweenService:Create(rock, TweenInfo.new(fadeTime, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {Transparency = 1}):Play()
...
function showHitboxMarkers(targetChar)
    ...
    table.insert(activeHitboxMarkers, box); task.delay(0.6, function() for _, marker in ipairs(activeHitboxMarkers) do pcall(function() marker:Destroy() end) end; activeHitboxMarkers = {} end)
end

-- ============================================================
-- LOCK SYSTEM (target, cámara, highlight)
-- ============================================================
...
local function getClosestLockTarget()
    ...
    local myChar = lplr.Character; local myRoot = myChar and myChar:FindFirstChild("HumanoidRootPart")
    for _, player in ipairs(Players:GetPlayers()) do
        ...
        local screenPos, onScreen = camera:WorldToScreenPoint(targetRoot.Position); if not onScreen then continue end
...
local function updateLockInfoGui()
    ...
    if flyanim.lockActive and flyanim.lockedTarget and flyanim.lockedTarget.Character and isTargetValidForLock(flyanim.lockedTarget) then
        ...
        if root then
            ...
            local displayName = flyanim.lockedTarget.DisplayName; if #displayName > 14 then displayName = displayName:sub(1,12)..".." end
            ...
            if healthLabel and humanoid then
                local health = math.floor(humanoid.Health); local maxHealth = humanoid.MaxHealth
            ...
            if heightLabel and myRoot then
                ...
                if heightDiff > 0 then
                    heightLabel.Text = "↓ "..heightDiff.." ".. (flyanim.FT.height_below or "studs abajo")
                    heightLabel.TextColor3 = Color3.fromRGB(150,220,255)
                elseif heightDiff < 0 then
                    heightLabel.Text = "↑ "..math.abs(heightDiff).." ".. (flyanim.FT.height_above or "studs arriba")
                    heightLabel.TextColor3 = Color3.fromRGB(255,200,100)
                    if flyanim.enabled and math.abs(heightDiff) >= 15 and flyanim.bg and flyanim.bg.Parent then
                        local toTargetH = Vector3.new(root.Position.X - myRoot.Position.X, 0, root.Position.Z - myRoot.Position.Z)
                        local behindDir = toTargetH.Magnitude > 0.1 and -toTargetH.Unit or Vector3.new(0,0,1)
                        local tpPos = root.Position + behindDir * 3
                        pcall(function() myRoot.CFrame = CFrame.new(tpPos, root.Position); myRoot.AssemblyLinearVelocity = Vector3.new(0,0,0); myRoot.AssemblyAngularVelocity = Vector3.new(0,0,0) end)
                    end
                else
                    heightLabel.Text = flyanim.FT.height_same or "↔ mismo nivel"
                    heightLabel.TextColor3 = Color3.fromRGB(150,255,150)
                end
            end
        end
        if flyanim.loadAvatarImage then flyanim.loadAvatarImage() end
    else
...
function toggleLock()
    ...
    if not flyanim.lockActive then
        ...
        if found and isTargetValidForLock(found) then flyanim.lockedTarget = found; flyanim.lockActive = true; applyLockIcon(found); updateLockHighlight(); if flyanim.loadAvatarImage then flyanim.loadAvatarImage() end end
...
function startLockSystem()
    ...
    local lockRenderConn = RunService.RenderStepped:Connect(function() updateLockInfoGui(); updateLockCamera(); updateLockHighlight() end)
...
-- ============================================================
-- MEGA UP (espacio mantenido)
-- ============================================================
local MegaUpLogic = {
    Active = false, LoopConn = nil, Token = 0,
}
...
-- ============================================================
-- LANDING WATCHER (anti-rebote al aterrizar)
-- ============================================================
...
local function startLandingWatcher()
    ...
    _landingWatchConn = RunService.Heartbeat:Connect(function()
        ...
        local char = lplr.Character; local hrp = char and char:FindFirstChild("HumanoidRootPart"); local hum = char and char:FindFirstChildOfClass("Humanoid")
        ...
        if hit then
            ...
            if animScriptPre then pcall(function() animScriptPre.Disabled = true end); task.delay(0.25, function() if not flyanim.waitingLand and animScriptPre and animScriptPre.Parent and (not flyanim.animScript or flyanim.animScript ~= animScriptPre) then pcall(function() animScriptPre.Disabled = false end) end end) end
...
-- ============================================================
-- SETUP DEL ANIMADOR Y TRACKS NORMALES
-- ============================================================
function setupAnimator(char)
    local animScript = char:FindFirstChild("Animate"); if animScript then animScript.Disabled = true; flyanim.animScript = animScript end
    local humanoid = char:WaitForChild("Humanoid", 5); if not humanoid then return end
    local animator = humanoid:WaitForChild("Animator", 5); if not animator then return end
    flyanim.animator = animator; flyanim.tracks = {}
    local ok, playing = pcall(function() return animator:GetPlayingAnimationTracks() end); if ok and playing then for _, t in ipairs(playing) do pcall(function() t:Stop(0) end) end end
    if flyanim.animConn then flyanim.animConn:Disconnect(); flyanim.animConn = nil end
    local hrp = char:WaitForChild("HumanoidRootPart", 5); if not hrp then return end
    local rj = hrp:WaitForChild("RootJoint", 5); if not rj then return end
    ...
    local function loadNorm(id, priority, looped)
        ...
        local ok2, t = pcall(function() return animator:LoadAnimation(anim) end); if not ok2 or not t then return nil end
        t.Priority = priority or Enum.AnimationPriority.Action3; t.Looped = (looped ~= false); return t
    end
    flyanim.normalTracks = {
    ...
    local ownAnimIds = {}; for _, id in pairs(ANIM) do ownAnimIds[id] = true end; ownAnimIds[CAIDA_BASE_ID] = true; ownAnimIds[CAIDA_OVERLAY_ID] = true
    flyanim.animConn = animator.AnimationPlayed:Connect(function(track) if not track or not track.Animation then return end; local id = track.Animation.AnimationId; if not ownAnimIds[id] then pcall(function() if track and track.IsPlaying then track:Stop(0) end end) end end)
    setupBlockTrack()
...
local function flyOn()
    ...
    stopLandingWatcher(); flyanim.waitingLand = false; flyanim.landingHeight = nil; flyanim.landingVelocity = 0; flyanim.landingVelocityCapture = 0
    _landAnimToken = _landAnimToken + 1; if _landBaseRef and pcall(function() _landBaseRef:Stop(0) end) then end; if _landOverlayRef and pcall(function() _landOverlayRef:Stop(0) end) then end; _landBaseRef = nil; _landOverlayRef = nil
    ...
    flyanim.gyroProtectionActive=false; flyanim.turboPreImpulsoActivo=false; flyanim.c0ControlToken=0; flyanim.c0HeightToken=0
    flyanim.noclipSpaceActive = false; flyanim.lastKnownPos = nil; flyanim.ragdollDetected = false
    flyanim.fKeyHeld = false; flyanim.blockCancelledByCombo = false; flyanim.lastSafePos = nil; flyanim.lastSafeTime = tick()
    if flyanim.gyroProtectionTimer then task.cancel(flyanim.gyroProtectionTimer) end
    if flyanim.backupGyro then flyanim.backupGyro:Destroy(); flyanim.backupGyro=nil end
    flyanim.isBlocking=false; flyanim.isSpaceAdv=false; flyanim._normalPlayId=0; flyanim.lastDamageTime = 0; flyanim.mouseHeld = false; flyanim.comboAnimStartTime = 0
    ...
    for _, t in pairs(flyanim.normalTracks) do pcall(function() if t and t.IsPlaying then t:Stop(0) end end) end; flyanim.normalTracks = {}
    for _, t in pairs(flyanim.tracks) do pcall(function() if t and t.IsPlaying then t:Stop(0) end end) end; flyanim.tracks = {}
    setupAnimator(char); _flyMakeMotors()
    startAntiImpulse(); startAnomalyProtection(); startTeleportGuard(); startBrakeSystem(); startMegaTurboUpListener(); startLockSystem()
    _flyBuildGui()
    flyanim._normalPlayId = 1; iniciarCicloNormal(1)
    _startMovementLoops()
end
...
local function flyOff()
    ...
    flyanim.brakingActive = false; flyanim.noclipSpaceActive = false; flyanim.noclipCtrlActive = false
    flyanim.isBlocking = false; flyanim.isSpaceAdv = false; flyanim.isBrazosActive = false
    flyanim.ragdollDetected = false; flyanim.comboPlaying = false; flyanim.comboBusy = false; flyanim.combo4Frozen = false
    flyanim.mouseHeld = false; flyanim.fKeyHeld = false; flyanim.blockCancelledByCombo = false
    flyanim.comboAnimStartTime = 0; flyanim.turboTransitioning = false; flyanim.megaTransitioning = false
    detenerNormalTracks(0); detenerPoseTurbo(); detenerPoseMega(); detenerEspacioAvanzado()
    stopBlocking(); stopParticleEmitter(); stopAntiImpulse(); stopBrakeSystem(); stopMegaTurboUpListener()
    stopAnomalyProtection(); stopComboC0Lock(); stopAnimBlockLoop(); stopTeleportGuard()
    setNoclip(false)
    if shakeConn then pcall(function() shakeConn:Disconnect() end); shakeConn=nil end
    destroyWhiteFlash()
    ...
    if flyanim.landingHeight < 10 then
    ...
    flyanim.landConn = hum.StateChanged:Connect(function(_, newState)
        ...
        if newState == Enum.HumanoidStateType.Landed or newState == Enum.HumanoidStateType.Running or newState == Enum.HumanoidStateType.RunningNoPhysics or newState == Enum.HumanoidStateType.Dead then
            ...
            if newState ~= Enum.HumanoidStateType.Dead then flyanim.landingHeight = capturedHeight; flyanim.landingVelocity = capturedVelocity; flyanim.landingVelocityCapture = capturedVelocity; doLanding(lplr.Character) else local animSc = char and char:FindFirstChild("Animate"); if animSc then animSc.Disabled = false end; flyanim.landingHeight = nil end
        end
    end)
    task.delay(10, function()
        if not flyanim.waitingLand then return end
        flyanim.waitingLand = false; if flyanim.landConn then flyanim.landConn:Disconnect(); flyanim.landConn=nil end
        local animSc = lplr.Character and lplr.Character:FindFirstChild("Animate"); if animSc then animSc.Disabled = false end
        local humFinal = lplr.Character and lplr.Character:FindFirstChildOfClass("Humanoid")
        if humFinal and humFinal.Parent then humFinal.PlatformStand = false; pcall(function() humFinal:ChangeState(Enum.HumanoidStateType.Running) end) end
        flyanim.landingHeight = nil
    end)
end

local function _startMovementLoops()
    if flyanim.tpMoveConn then flyanim.tpMoveConn:Disconnect(); flyanim.tpMoveConn = nil end
    ...
    flyanim.tpMoveConn = RunService.Stepped:Connect(function(_, dt)
        ...
        local cc = lplr.Character; local root2 = cc and cc:FindFirstChild("HumanoidRootPart"); if not root2 then return end
    ...
    flyanim.rsConn = RunService.RenderStepped:Connect(function(dt)
        ...
        local cc = lplr.Character; local root = cc and cc:FindFirstChild("HumanoidRootPart"); local ch = cc and cc:FindFirstChildOfClass("Humanoid")
...
local function _flyBuildGui()
    ...
    local rootFrame = Instance.new("Frame", sg)
    rootFrame.Size=UDim2.new(0,W,0,H_FULL); rootFrame.Position=UDim2.new(0.5,-W/2,0,6)
    rootFrame.BackgroundTransparency=1; rootFrame.BorderSizePixel=0
    ...
    titleLbl.Text=flyanim.FT.fly_title.." ["..flyanim.flyKey.Name.."]"; titleLbl.TextXAlignment=Enum.TextXAlignment.Left
    ...
    modeLbl.TextSize=12; modeLbl.TextColor3=C_GREEN; modeLbl.Text=flyanim.FT.mode_normal; modeLbl.TextXAlignment=Enum.TextXAlignment.Center
    ...
    lockLabel.Text=flyanim.FT.lock_label.." ["..flyanim.lockKey.Name.."]"; lockLabel.TextXAlignment=Enum.TextXAlignment.Left
    ...
    lockHint.Text=flyanim.FT.lock_hint_prefix..flyanim.lockKey.Name; lockHint.TextXAlignment=Enum.TextXAlignment.Right
    ...
    noclipTitle.TextSize=11; noclipTitle.TextColor3=C_TEXT; noclipTitle.Text=flyanim.FT.noclip_title; noclipTitle.TextXAlignment=Enum.TextXAlignment.Left
    ...
    spaceLabel.TextSize=10; spaceLabel.TextColor3=C_TEXT; spaceLabel.Text=flyanim.FT.noclip_space; spaceLabel.TextXAlignment=Enum.TextXAlignment.Left
    ...
    ctrlLabel.TextSize=10; ctrlLabel.TextColor3=C_TEXT; ctrlLabel.Text=flyanim.FT.noclip_ctrl; ctrlLabel.TextXAlignment=Enum.TextXAlignment.Left
    ...
    megaLabel.TextSize=10; megaLabel.TextColor3=C_TEXT; megaLabel.Text=flyanim.FT.noclip_mega; megaLabel.TextXAlignment=Enum.TextXAlignment.Left
    ...
    local speedBox = makeRow(flyanim.FT.speed_base,       6,  BASE_SPEED,  1, 10000)
    local fastBox  = makeRow(flyanim.FT.speed_turbo_mult, 30, FAST_MULT,   1, 1000)
    local turboBox = makeRow(flyanim.FT.speed_mega_mult,  54, TURBO_MULT,  1, 1000)
    ...
    resetBtn.TextColor3=C_TEXT; resetBtn.Text=flyanim.FT.speed_reset
    ...
    local function setExpanded(state)
        ...
        rootFrame.Size = UDim2.new(0,W,0,h)
    ...
    topBar.InputBegan:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.MouseButton1 or inp.UserInputType==Enum.UserInputType.Touch then
            dragging=true; dragStartMouse=Vector2.new(inp.Position.X,inp.Position.Y); dragStartPos=rootFrame.Position
        end
    end)
    ...
    UserInputService.InputChanged:Connect(function(inp)
        ...
        rootFrame.Position = UDim2.new(dragStartPos.X.Scale, dragStartPos.X.Offset+delta.X, dragStartPos.Y.Scale, dragStartPos.Y.Offset+delta.Y)
    end)

    flyanim.updateLbl = function(keyName) pcall(function() titleLbl.Text=flyanim.FT.fly_title.." ["..keyName.."]" end) end
    flyanim.updateMode = function(modeName)
        pcall(function()
            ...
            local labels  = { normal=flyanim.FT.mode_normal, fast=flyanim.FT.mode_fast..tostring(multF), turbo=flyanim.FT.mode_mega..tostring(multT), megaup=flyanim.FT.mode_megaup..tostring(multT) }
    ...
    local function createLockInfoGui(parentFrame)
        ...
        flyanim.loadAvatarImage = function()
            if not flyanim.lockInfoGui then return end
            local playerImg = flyanim.lockInfoGui:FindFirstChild("PlayerImgContainer") and flyanim.lockInfoGui.PlayerImgContainer.PlayerImage
    ...
    createLockInfoGui(rootFrame)
end

-- ============================================================
-- CONEXIONES GLOBALES DE ENTRADA Y CHARACTER ADDED
-- ============================================================
...
local function _connectGlobal()
    ...
    _inputConn = UserInputService.InputBegan:Connect(function(input, gpe)
        ...
        if input.KeyCode == flyanim.flyKey then
            if flyanim.enabled then flyOff() else flyOn() end
            return
        end

        if input.KeyCode == Enum.KeyCode.Q then
            if not flyanim.enabled then return end
            local wDown = UserInputService:IsKeyDown(Enum.KeyCode.W); local sDown = UserInputService:IsKeyDown(Enum.KeyCode.S)
            local aDown = UserInputService:IsKeyDown(Enum.KeyCode.A); local dDown = UserInputService:IsKeyDown(Enum.KeyCode.D)
            if not wDown then
                if sDown and not aDown and not dDown then
                    triggerDash("back", flyanim.BACK_DASH_SPEED, flyanim.DASH_DURATION * 1.2)
                    local t = getTrack("rbxassetid://82718659527221")
                    if t then
                        for id, track in pairs(flyanim.tracks) do
                            if id ~= "rbxassetid://82718659527221" and track and track.IsPlaying then pcall(function() track:Stop(0.04) end) end
                        end
                ...
                if dDown and not sDown and not aDown then
                    ...
                    local t = getTrack("rbxassetid://93831708296422")
                    if t then
                        for id, track in pairs(flyanim.tracks) do
                            if id ~= "rbxassetid://93831708296422" and track and track.IsPlaying then pcall(function() track:Stop(0.04) end) end
                        end
                        pcall(function() t:Play(0.04, 1, 1); t:AdjustSpeed(3.0) end)
                    end
                    return
            ...
            if flyanim.mode == "normal" then
                ...
                flyanim.mode = "fast"
                flyanim.speed = flyanim.BASE_SPEED * flyanim.FAST_MULT
            ...
            if flyanim.mode == "fast" then
                if tick() - flyanim.turboActivatedAt >= 0.75 then
                    flyanim.mode = "turbo"
                    flyanim.speed = flyanim.BASE_SPEED * flyanim.TURBO_MULT
        ...
        if input.KeyCode == Enum.KeyCode.F and flyanim.enabled then
            ...
            if not flyanim.blockCancelledByCombo then
                if flyanim.mode == "normal" and not flyanim.turboPreImpulsoActivo and not flyanim.megaTurboUpActive then
                    if flyanim.comboPlaying then
                        flyanim.comboToken = flyanim.comboToken + 1
                        for _, track in pairs(flyanim.tracks) do pcall(function() if track and track.IsPlaying then track:Stop(0.08) end) end
                        flyanim.comboPlaying = false; flyanim.comboBusy = false; flyanim.combo4Frozen = false; resetCombo(); stopComboC0Lock()
                    end
                    startBlocking()
                end
            end
        ...
        if input.KeyCode == Enum.KeyCode.Space and flyanim.enabled and flyanim.mode == "normal" then
            if not flyanim.comboPlaying and not flyanim.turboPreImpulsoActivo then manejarEspacioPresionadoNormal() end
        end
    ...
    _charConn = flyanim.lplr.CharacterAdded:Connect(function(newChar)
        if shakeConn then pcall(function() shakeConn:Disconnect() end); shakeConn=nil end
        destroyWhiteFlash(); if camera then camera.FieldOfView=70 end
        ...
        flyanim.idleWatchToken = flyanim.idleWatchToken + 1; flyanim.c0ControlToken = 0; flyanim.c0HeightToken = 0
        flyanim.lastDamageTime = 0; flyanim.straightLineActive = false; flyanim.noclipSpaceActive = false
        flyanim.noclipCtrlActive = false; flyanim.mouseHeld = false; flyanim.lastKnownPos = nil
        flyanim.ragdollDetected = false; flyanim.fKeyHeld = false; flyanim.blockCancelledByCombo = false
        flyanim.comboAnimStartTime = 0; flyanim.lastSafePos = nil; flyanim.lastSafeTime = tick()
        flyanim.isTeleportGuardActive = false
        if flyanim.idleAnimConn then flyanim.idleAnimConn:Disconnect(); flyanim.idleAnimConn=nil end
        flyanim.lateralKilled=true; flyanim.atrasKilled=true
        if flyanim.spaceHoldTimerAdv then task.cancel(flyanim.spaceHoldTimerAdv); flyanim.spaceHoldTimerAdv=nil end
        flyanim.espacioTrackActivo=nil
        flyanim.dashAnimToken = flyanim.dashAnimToken + 1
        flyanim.sessionToken = flyanim.sessionToken + 1
        _landAnimToken = _landAnimToken + 1; _landBaseRef = nil; _landOverlayRef = nil
        stopParticleEmitter(); stopBrakeSystem(); stopAntiImpulse(); stopMegaTurboUpListener(); stopLockSystem()
        stopAnomalyProtection(); stopComboC0Lock(); stopAnimBlockLoop(); stopTeleportGuard()
        detenerWatchdogAltura()
        if flyanim.turboRenderConn then flyanim.turboRenderConn:Disconnect(); flyanim.turboRenderConn=nil end
        if flyanim.megaRenderConn then flyanim.megaRenderConn:Disconnect(); flyanim.megaRenderConn=nil end
...
-- ============================================================
-- ANIM BLOCK LOOP (evita animaciones externas)
-- ============================================================
local function startAnimBlockLoop()
    ...
    flyanim.animBlockRenderConn = RunService.RenderStepped:Connect(function()
        if flyanim.sessionToken ~= mySession then if flyanim.animBlockRenderConn then flyanim.animBlockRenderConn:Disconnect(); flyanim.animBlockRenderConn = nil end; return end
        if not flyanim.enabled then if flyanim.animBlockRenderConn then flyanim.animBlockRenderConn:Disconnect(); flyanim.animBlockRenderConn = nil end; return end
        local anim = flyanim.animator; if not anim then return end
        pcall(function()
...
-- ============================================================
-- DAMAGE DETECTOR (para bloqueo)
-- ============================================================
function setupDamageDetector()
    if flyanim.damageConn then flyanim.damageConn:Disconnect(); flyanim.damageConn = nil end
    local char = lplr.Character; if not char then return end
...
-- ============================================================
-- IDLE WATCHER (vuelve a normal si no hay teclas)
-- ============================================================
...
-- ============================================================
-- BYPASS TEMPORAL (para otros módulos del hub)
-- ============================================================
local bypassToken = 0
function flyanim.Bypass(duration, reason)
    if not flyanim.enabled then return end
    bypassToken = bypassToken + 1
    local myToken = bypassToken
    flyOff()
    task.delay(duration, function()
        if bypassToken == myToken and lplr.Character then flyOn() end
    end)
end
...
function M.Start(lplrRef, flyKeyRef)
    ...
    flyanim.FT = reloadText()  -- actualizar idioma
    _flyBuildGui()
    startIdleWatcher()
    startComboListener()
    _connectGlobal()
...
function M.Stop()
    ...
    local char = lplr.Character
    if char then
        local animScript = char:FindFirstChild("Animate"); if animScript then animScript.Disabled = false end
        local root = char:FindFirstChild("HumanoidRootPart")
...
function M.Toggle(state)
    if state then
        if not flyanim.enabled then flyOn() end
    else
        if flyanim.enabled then flyOff() end
    end
end
...
function M.SetKey(keyCode)
    ...
    if flyanim.updateLbl then pcall(function() flyanim.updateLbl(keyCode.Name) end)
    if _inputConn then _inputConn:Disconnect(); _inputConn = nil end
    if _inputConn_End then _inputConn_End:Disconnect(); _inputConn_End = nil end
    _connectGlobal()
end
...
function M.SetLockKey(keyCode)
    ...
    if flyanim.lockLabels then
        flyanim.lockLabels.label.Text = flyanim.FT.lock_label.." ["..keyCode.Name.."]"
        flyanim.lockLabels.hint.Text  = flyanim.FT.lock_hint_prefix..keyCode.Name
    end
    if flyanim.lockConn then flyanim.lockConn:Disconnect(); flyanim.lockConn = nil end
...
function M.Bypass(duration, reason) return flyanim.Bypass(duration, reason) end
